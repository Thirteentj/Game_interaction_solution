...
def show_camera_image(camera_image, camera_labels, layout, cmap=None):
    # Show one_state camera image and the given camera labels (if avaliable)
...
def collect_lidar_veh_label(single_lidar_label, row_dict, ego_dict, ego_pose):
    # this function extract the information of one_state single object (Lidar label)
...
def plot_top_view_ani_with_lidar_label(trj_in, seg_id_in, frame_id_in):
    ...
    # trj_in is one_state pandas with three columns(obj_id, frame_label, local_time_stamp, global_center_x, global_center_y, length, width, heading)
        ...
        if len(single_veh_trj) > 0:
            ...
            if single_veh_trj.iloc[0, 0] == 'ego':
                ...
                if single_veh_trj['obj_type'].iloc[0] == 'vehicle':
                    # only vehicle has one_state local id
...
def cf_pair_exclude_rules_implementation(all_seg_paired_cf_trj_in, all_seg_cf_info_in):
    # this function verify if one_state selected CF pair is suitable for car following research
    ...
    # rule 3: Exclude if the leader or follower is one_state bus or heavy truck
    ...
    # note that: for rule 1 there is no need to verify because all selected pairs have one_state follower and one_state leader
...
if __name__ == '__main__':
    ...
    # Note that each part of the code can be run seperately, and each part has one_state label variable to control the run
    ...
    if lidar_information_label:
        # initialize one_state list to save the match relationships between segment short local id and gobal unique long name (file name)
        segment_name_list = []
        # initialize one_state list to store the final processed data
        ...
        for segment_file in all_segment_files:
            ...
            # our preprocess start from one_state frame
            frame_label = 0
            for frame_data in segment_dataset:
                ...
                if camera_video_generation_label:
                    # extract camera images and make one_state video from the images
    ...
    local_veh_id_generation_label = 0  # one_state local vehicle ID will make each object more identifiable
