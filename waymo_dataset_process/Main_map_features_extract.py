...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(file_index_list))):
        ...
        df_single_seg_all_intersection_info, df_single_seg_all_scenario_lane_topo_info = Ext.get_intersection_info(
            segment_dataset, df_turn_left_scenario,file_index, seg_trj, length, test_state, target_scenario)
        ...
        outpath_lane_topo_info = 'data/all_lane_topo_info/' + file_index + '_all_lane_topo_info_test_0206' + '.csv'  # 现在是一个scenario导出一个csv,后面需要导出一个完整的
        df_single_seg_all_scenario_lane_topo_info.to_csv(outpath_lane_topo_info)

        # outpath_intersection_info = 'data/all_lane_topo_info/' + file_index + '_all_intersection_info_test_0206' + '.csv'
        # df_single_seg_all_intersection_info.to_csv(outpath_intersection_info)
        #------------交叉口信息提取结束-------------
    ...
    # outpath_intersection_info = 'data/' + 'all_seg_all_intersection_info' + '.csv'
