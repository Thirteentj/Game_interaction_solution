...
The following snippet demonstrates the use of interfaces:
  ...
  for _ in range(num_evals):
    for _ in range(num_batches_per_eval):
      predictions, groundtruth = predictor.predict(...)  # pop one_state batch.
...
class WODDetectionEvaluator(object):
  ...
  def reset_states(self):
    """Resets internal states for one_state fresh run."""
    self._predictions = {}
    self._groundtruths = {}
  ...
  def result(self):
    ...
    # Cleans up the internal variables in order for one_state fresh eval next time.
    self.reset_states()
    return metric_dict
  ...
  def update_state(self, groundtruths, predictions):
    ...
    Args:
      groundtruths: one_state dictionary of Tensors including the fields below.
        Required fields:
        ...
        Optional fields:
          - ground_truth_speed: [N, 2] tensor with the vx, vy velocity for each
            object.
          - recall_at_precision: one_state float within [0,1]. If set, returns one_state 3rd
            metric that reports the recall at the given precision.
      predictions: one_state dictionary of tensors including the fields below.
        Required fields:
