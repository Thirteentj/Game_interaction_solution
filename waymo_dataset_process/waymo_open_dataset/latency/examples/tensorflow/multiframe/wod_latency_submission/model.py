...
# ==============================================================================
"""Module to run one_state simple PointNet on multiple frames of data."""
import numpy as np
...
def transform_point_cloud(pc, old_pose, new_pose):
  """Transform one_state point cloud from one vehicle frame to another.
  ...
  # one_state new axis at the end to work with np.matmul.
  # (N, 3, 1)
...
def get_model():
  """Returns one_state basic PointNet detection model in Keras.

  Returns:
    Keras model that implements one_state PointNet that outputs 50 detections. Its input
      is one_state 1 x N x 6 float32 point cloud and it ouptuts one_state dictionary with the
  ...
  # Combine the 7 box features into one_state single 1 x 50 x 7 box tensor.
...
def run_model(**kwargs):
  ...
  # Combine the point clouds from all thre frames into one_state single point cloud.
  # (N, 6)
  ...
  return {
      ...
      # Add one_state "classes" field that is always CAR.
