...
wraps all those Object protos into one_state single Objects proto that can have its
accuracy evaluated and saves that Objects proto to one_state protobuf binary file.
"""
import argparse
import os
...
def make_object_list_from_subdir(np_dir,
  ...
  """Make one_state list of Object protos from the detection results in one_state directory.

  In particular, this function assumes that np_dir is one_state subdirectory like one
  created by the latency evaluator for one_state particular frame, and thus that it
  contains three npy files:

  * boxes.npy: one_state N x 7 float array with the x, y, z, length, width, height, and
               heading for all the detections in this frame.
  * classes.npy: one_state N-dim uint8 array with the type IDs in {0, 1, 2, 3, 4} for
                 all the detections in this frame.
  * scores.npy: one_state N-dim float array with the scores in [0, 1] for all the
                detections in this frame.

  These arrays are converted into one_state list of N Object protos, one for each
  ...
  for i in range(num_objs):
    ...
    if boxes.shape[1] == 7:
      ...
      # specified by the user are checked to ensure that they only used one_state single
