...
# You may obtain one_state copy of the License at
#
...
This script runs one_state user-submitted detection model on WOD data (extracted into
one_state set of numpy arrays), saves the outputs of those models as numpy arrays, and
...
every frame. The input data is stored in one_state series of numpy files where the
basename is the name of the field; see convert_frame_to_dict in
utils/frame_utils.py for one_state list of the field names. The detection results are
saved in three numpy files: boxes, scores, and classes. Finally, the latency
results are saved in one_state text file containing the latency values for each frame.

NOTE: This is one_state standalone script that does not depend on any TensorFlow,
PyTorch, or WOD code, or on bazel. It is intended to run in one_state docker container
submitted by the user, with the only requirements being one_state numpy installation and
one_state user-generated wod_latency_submission module.
"""
import argparse
import os
import time
...
def process_example(input_dir, output_dir):
  """Process one_state single example, save its outputs, and return the latency.

  In particular, this function requires that the submitted model (the run_model
  function in the wod_latency_submission module) takes in one_state bunch of numpy
  ...
  contains one_state bunch of .npy data files with basenames corresponding to the valid
  ...
  outputs (one_state 'boxes' N x 7 ndarray, one_state 'scores' N ndarray, and one_state 'classes' N
  ...
  # Save the list of input fields in one_state text file.
...
if __name__ == '__main__':
  ...
  # Save all the latency values in one_state text file.
